...
2. **Self-hostable stack, not vendor-locked.** Vercel and Supabase are
   ...
   one-click deploy this on Railway." [open-seo](https://github.com/every-app/open-seo)
   and [elmo](https://github.com/elmohq/elmo) both converged on plain
   Postgres + Drizzle + Docker for exactly this reason — followed here.

## Stack

| Layer | Choice | Why |
|---|---|---|
| Monorepo | pnpm workspaces + Turborepo | Matches both reference repos; page-analyzer needs in-process access to crawler, SERP, AI-visibility, and GSC/GA4 modules simultaneously — not worth splitting into separately-versioned repos |
| App framework | Next.js (App Router) | Single deploy artifact for dashboard UI + API routes; supports Docker standalone output for Railway. Both open-seo and elmo use TanStack Start instead — considered switching, decided against it: their reason (Cloudflare Workers portability via Vinxi) doesn't apply since we're Railway/Docker-only, and Next.js has deeper ecosystem maturity + lower implementation risk for an AI-assisted solo build |
| API layer | Next.js Route Handlers, no separate Express/NestJS | Confirmed both reference projects also skip a standalone API framework (they use TanStack Start server functions instead) — validates not running a second HTTP service |
...
| Background jobs | pg-boss, run in a separate `apps/worker` process | Confirmed against elmo's actual source (`apps/worker`, `pg-boss@^12`) — Postgres-native, no extra infra, matches our self-host-first goal |
...
- `apps/web` — Next.js server. Handles all synchronous requests: dashboard
  ...
  immediately.
- `apps/worker` — plain Node process (no HTTP framework at all, mirroring
  elmo's `apps/worker`). Consumes `pg-boss` jobs and calls into the
  `packages/*` domain packages to do the actual work.
...
Neither of the reference repositories (`open-seo` and `elmo`) use Redis, and this project follows that lead. To maintain self-host simplicity and reduce infrastructure overhead, Postgres-based caching (or simple in-memory/file caching where applicable) is preferred over running a separate Redis instance. This ensures the app can run easily on any Node + Postgres setup.
