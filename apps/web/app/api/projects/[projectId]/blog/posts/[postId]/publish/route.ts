...
import { scheduleBlogPost } from "@/lib/publish/schedule-post";

type RouteParams = { projectId: string; postId: string };

// POST body: { scheduledFor?: string } - "Publish now" (omit scheduledFor)
// and "Schedule for later" both hit this; see lib/publish/schedule-post.ts
// for why they're the same code path.
export const POST = withAuth<RouteParams>(async (req, ctx, session) => {
  ...
  try {
    const result = await scheduleBlogPost(session.user.id, projectId, postId, scheduledFor);
