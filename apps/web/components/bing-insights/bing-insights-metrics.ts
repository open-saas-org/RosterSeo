// Summary-metric helper for the Bing Insights page - same period-over-period
...
function sumBy(rows: BingPerformanceRow[], key: "clicks" | "impressions"): number {
  return rows.reduce((acc, row) => acc + row[key], 0);
}

function avgBy(rows: BingPerformanceRow[], key: "ctr" | "avgClickPosition"): number {
  return rows.length ? rows.reduce((acc, row) => acc + row[key], 0) / rows.length : 0;
}

export function summarizeBingRows(rows: BingPerformanceRow[]): BingInsightsMetrics {
  ...
  const avgPosition = avgBy(rows, "avgClickPosition");
  ...
  const positionChange = avgBy(currRows, "avgClickPosition") - avgBy(prevRows, "avgClickPosition");
