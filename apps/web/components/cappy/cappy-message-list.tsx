...
import { CappyToolConfirmCard } from "./cappy-tool-confirm-card";
import { TOOL_LABELS } from "./tool-labels";
import type { CappyLiveStatus, CappyMessageView, CappyToolCallView } from "./types";

function ToolCallDisclosure({ call }: { call: CappyToolCallView }) {
...
function LiveStatusRow({ status }: { status: CappyLiveStatus }) {
  const label = status.state === "thinking" ? "Cappy is thinking…" : `Using ${TOOL_LABELS[status.name] ?? status.name}…`;
  return (
...
export function CappyMessageList({
  messages,
  status,
  onRespondToToolCall,
}: {
  messages: CappyMessageView[];
  status?: CappyLiveStatus | null;
                ...
                <div className="flex w-full flex-col gap-1.5">
                  {m.toolCalls.map((call) =>
                    call.status === "pending_confirmation" ? (
                      <CappyToolConfirmCard key={call.id} call={call} onRespond={onRespondToToolCall} />
                    ) : (
