...
export type CappySettings = { provider: string; model: string; configuredProviders: string[] };

export function CappySettingsDialog({
  projectId,
  open,
  onOpenChange,
  settings,
  ...
  settings: CappySettings | null;
  onSettingsChange: (next: CappySettings) => void;
}) {
  ...
  async function handleSave() {
    ...
    setIsSaving(true);
    try {
      const res = await fetch(`/api/projects/${projectId}/cappy/settings`, {
        method: "PATCH",
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          <DialogTitle>Cappy settings</DialogTitle>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="cappy-model" className="text-xs text-muted-foreground">
              Model (optional override)
            </Label>
            <Input id="cappy-model" placeholder="Provider default" value={model} onChange={(e) => setModel(e.target.value)} />
          </div>
          {error ? (
