...
import type { CappyConversationView } from "./types";
...
// that CappyWorkspace (a separate part of the tree, on the page itself) may
// have just created a new conversation or renamed one from its first message.
export function CappySidebarNav({ projectId }: { projectId: string }) {
  const searchParams = useSearchParams();
  const activeId = searchParams.get("c");
  const [conversations, setConversations] = useState<CappyConversationView[]>([]);

  useEffect(() => {
    let cancelled = false;
    fetch(`/api/projects/${projectId}/cappy/conversations`)
  ...
  return (
    <div className="flex flex-col gap-0.5 px-2">
      ...
      {conversations.length === 0 ? (
        <p className="px-2.5 py-2 text-xs text-sidebar-foreground/60">No chats yet - ask Cappy something to start one.</p>
      ) : (
        conversations.map((c) => (
          <Link
            key={c.id}
