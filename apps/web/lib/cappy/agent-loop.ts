...
import { cappyConversations, cappyMessages, projects, withUserContext, type CappyToolCall } from "@rosterseo/db";
import { getProvider, type CappyToolDef } from "@rosterseo/ai-visibility";
import { buildCappyContext } from "./context";
import { CAPPY_TOOLS, executeCappyTool, getCappyTool, type CappyProject } from "./tools/registry";
import { resolveCappyModel, resolveCappyProvider } from "./provider";
import { maybeRefreshProjectNotes } from "./project-notes";

// Per-turn cap on read-tool round trips before Cappy is forced to stop and
...
export type CappyMessageRow = typeof cappyMessages.$inferSelect;
export type CappyTurnResult = { messages: CappyMessageRow[]; pendingConfirmation: boolean };
...
export type CappyTurnEvent =
  | { type: "status"; state: "thinking" }
  | { type: "status"; state: "tool"; name: string }
  | { type: "message"; message: CappyMessageRow }
  | { type: "done"; pendingConfirmation: boolean };

export type CappyTurnEmit = (event: CappyTurnEvent) => void;

const TOOL_DEFS: CappyToolDef[] = CAPPY_TOOLS.map((t) => ({ name: t.name, description: t.description, parameters: t.parameters }));

async function insertMessage(userId: string, projectId: string, conversationId: string, role: "user" | "assistant", content: string | null, toolCalls: CappyToolCall[] | null) {
  const [row] = await withUserContext(userId, (tx) =>
    tx.insert(cappyMessages).values({ conversationId, projectId, role, content, toolCalls }).returning(),
  );
  await withUserContext(userId, (tx) => tx.update(cappyConversations).set({ lastMessageAt: new Date() }).where(eq(cappyConversations.id, conversationId)));
  return row!;
}
...
// whatever's currently in cappy_messages for the conversation.
export async function runCappyTurn(userId: string, projectId: string, conversationId: string, emit?: CappyTurnEmit): Promise<CappyTurnResult> {
...
export async function resumeCappyTurn(
  userId: string,
  projectId: string,
  ...
  approve: boolean,
  emit?: CappyTurnEmit,
): Promise<CappyTurnResult> {
  const messageRow = await withUserContext(userId, async (tx) => {
    const [row] = await tx.select().from(cappyMessages).where(eq(cappyMessages.conversationId, conversationId)).orderBy(desc(cappyMessages.createdAt)).limit(1);
    return row ?? null;
  });
  ...
  const project = await loadCappyProject(userId, projectId);

  if (approve) {
    emit?.({ type: "status", state: "tool", name: target.name });
    const execResult = await executeCappyTool(target.name, target.arguments, { userId, projectId }, project);
  ...
  await withUserContext(userId, (tx) => tx.update(cappyMessages).set({ toolCalls: updatedCalls }).where(eq(cappyMessages.id, messageRow.id)));
...
async function loadCappyProject(userId: string, projectId: string): Promise<CappyProject> {
...
async function continueLoop(userId: string, projectId: string, conversationId: string, iterationsSoFar: number, emit?: CappyTurnEmit): Promise<CappyTurnResult> {
  const ctx = await buildCappyContext(userId, projectId, conversationId);
  const providerId = resolveCappyProvider(ctx.project.cappyProvider);
  const model = resolveCappyModel(ctx.project.cappyModel, providerId);
  ...
  if (result.stopReason !== "tool_calls" || result.toolCalls.length === 0) {
    ...
    emit?.({ type: "message", message: row });
    void maybeRefreshProjectNotes(userId, projectId).catch((err) => console.error("[cappy] project-notes refresh failed", err));
    emit?.({ type: "done", pendingConfirmation: false });
    return { messages: [row], pendingConfirmation: false };
  }
  ...
  const project: CappyProject = { id: ctx.project.id, domain: ctx.project.domain, name: ctx.project.name };
  const toolCallEntries: CappyToolCall[] = [];

  for (const call of result.toolCalls) {
    const spec = getCappyTool(call.name);
    if (!spec) {
    ...
    const execResult = await executeCappyTool(call.name, call.arguments, { userId, projectId }, project);
