import type { CappyToolDef } from "@rosterseo/ai-visibility";
import * as read from "./read";
import * as write from "./write";

export type CappyToolContext = { userId: string; projectId: string };
export type CappyProject = { id: string; domain: string; name: string };

export type CappyToolSpec = CappyToolDef & { requiresConfirmation: boolean };

// Every tool Cappy can call, in one place - real read tools (mirroring
...
// comment in write.ts). No `projectId` param on any of these - Cappy is
// always scoped to whichever project the chat panel is open on
// (CappyToolContext), never asked-for by the model.
export const CAPPY_TOOLS: CappyToolSpec[] = [
  {
  ...
  },
  {
    ...
    description: "Update Cappy's own standing memory notes for this project - call this when you learn something worth remembering for future conversations.",
...
const TOOLS_BY_NAME = new Map(CAPPY_TOOLS.map((t) => [t.name, t]));

export function getCappyTool(name: string): CappyToolSpec | undefined {
  return TOOLS_BY_NAME.get(name);
}

export type CappyToolExecutionResult = { ok: true; result: unknown } | { ok: false; error: string };
...
export async function executeCappyTool(name: string, args: Record<string, unknown>, ctx: CappyToolContext, project: CappyProject): Promise<CappyToolExecutionResult> {
  try {
...
async function dispatch(name: string, args: Record<string, unknown>, ctx: CappyToolContext, project: CappyProject): Promise<unknown> {
  switch (name) {
