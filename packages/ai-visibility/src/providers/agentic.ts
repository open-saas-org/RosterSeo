...
// Cappy (the in-app AI assistant, apps/web/lib/cappy/*) is this package's one
...
// apps/mcp-server/src/index.ts's TOOLS array already uses, so Cappy's real
// tool catalog (apps/web/lib/cappy/tools/registry.ts) can be handed to
// either format converter below with no translation step.
export interface CappyToolDef {
  name: string;
  description: string;
  parameters: Record<string, unknown>;
}

export interface CappyToolCallRequest {
  id: string;
  name: string;
...
export type AgenticMessage =
  ...
  | { role: "assistant"; content: string | null; toolCalls?: CappyToolCallRequest[] }
...
export interface AgenticChatResult {
  content: string | null;
  toolCalls: CappyToolCallRequest[];
  modelVersion?: string;
  usage?: TokenUsage;
  costUsd?: number;
...
export function toOpenAiCompatibleTools(tools: CappyToolDef[]) {
  return tools.map((t) => ({
...
export function fromOpenAiCompatibleResponse(data: any): AgenticChatResult {
  ...
  const toolCalls: CappyToolCallRequest[] = rawToolCalls.map((tc: any) => {
...
export function toAnthropicTools(tools: CappyToolDef[]) {
...
export function fromAnthropicResponse(data: any): AgenticChatResult {
  ...
  const toolCalls: CappyToolCallRequest[] = blocks
